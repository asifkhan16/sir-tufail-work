...
class get_data{
	...
	function get_students(){
		...
		$query = "SELECT s.*, c.class_name FROM students s JOIN class c ON s.class_id = c.class_id ORDER BY s.student_id DESC";
	...
	function add_student(){
		...
		$check = $this->db->prepare("SELECT student_id FROM students WHERE student_email = ? LIMIT 1");
		$check->execute( array($email) );
		if ($check->rowCount() == 1){
			return "Email already registered";
		}

		$query = "INSERT INTO students(student_name, student_email, student_contact, reg_no, class_id) VALUES(?,?,?,?,?)";
		$stmt = $this->db->prepare($query);
	...
	function update_student(){
		...
		$query = "UPDATE students SET student_name = ?, student_email = ?, student_contact = ?, reg_no = ?, class_id = ? WHERE student_id = ? LIMIT 1";
